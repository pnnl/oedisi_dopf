...
class ModelUtil:
    def __init__(self, dss):
        ...
        for bus in dss.Circuit.AllBusNames():
            ...
            self.bus_info[bus.upper()] = {
                "nodes": self.dss.Bus.Nodes(),
                "basekV": self.dss.Bus.kVBase(),
                "loc": [],
            }
    ...
    def find_network_graph(self):
        ...
        while flag > 0:
            ...
            if "Transformer" in self.dss.Topology.BranchName():
                self.branch_info[self.dss.Topology.BranchName()] = {
                    "bus1": bus1,
                    "bus2": bus2,
                }
        ...
        print(
            "Edges: {} and Nodes: {}".format(G.number_of_edges(), G.number_of_nodes())
        )
    ...
    def grid_voltage_profile(self):
        ...
        for line in line_flow_sensors:
            ...
            print(
                complex(
                    self.dss.CktElement.Powers()[0], self.dss.CktElement.Powers()[1]
                )
            )
            print(
                complex(
                    self.dss.CktElement.Powers()[2], self.dss.CktElement.Powers()[3]
                )
            )
            print(
                complex(
                    self.dss.CktElement.Powers()[4], self.dss.CktElement.Powers()[5]
                )
            )
