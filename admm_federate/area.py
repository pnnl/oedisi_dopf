...
def graph_process(branch_info: dict):
    ...
    for b in branch_info:
        if branch_info[b]["type"] != "SWITCH":
            graph.add_edge(branch_info[b]["fr_bus"], branch_info[b]["to_bus"])
        else:
            edges.append([branch_info[b]["fr_bus"], branch_info[b]["to_bus"]])
    return graph, edges
...
def area_info(branch_info: dict, bus_info: dict, source_bus: str):
    ...
    for key, val_bus in branch_info.items():
        if (
            val_bus["fr_bus"] in bus_info_area_i
            and val_bus["to_bus"] in bus_info_area_i
        ):
            if branch_info[key]["type"] not in secondary_model and key not in nor_open:
                ...
                branch_sw_data_area_i[key]["from"] = bus_info_area_i[
                    branch_info[key]["fr_bus"]
                ]["idx"]
                branch_sw_data_area_i[key]["to"] = bus_info_area_i[
                ...
                if branch_info[key]["type"] == "SPLIT_PHASE":
                    branch_sw_data_area_i[key]["impedance"] = branch_info[key][
                        "impedance"
                    ]
                    branch_sw_data_area_i[key]["impedance1"] = branch_info[key][
                        "impedance1"
                    ]
                else:
