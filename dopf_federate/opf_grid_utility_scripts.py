from scipy.sparse import csc_matrix, linalg, load_npz, save_npz, hstack, vstack
# import gc
...
def unpack_fdrvals_mats(opfobj):
    ...
    # opfobj.y_matrix = load_npz("y_matrix.npz")
    ...
    #
    # opfobj.y_line = load_npz("y_line.npz")
    #
...
def unpack_fdrvals_vecs(opfobj):
    ...
    opfobj.p_loss_dss = 289425.98
    opfobj.q_loss_dss = 178886.24
    ...
    if len(opfobj.cap_info.adj_matrix) > 0:
        ...
        opfobj.cap_load_inc_matrix = np.array([])
        opfobj.cap_bank_init = np.array([])

    if len(opfobj.tap_info.adj_matrix) > 0:
        ...
        opfobj.reg_inc_matrix = np.array([])
        opfobj.reg_taps_init = np.array([])
    ...
    opfobj.pv_inc_matrix = load_npz('pv_incidence_matrix.npz')
    opfobj.pv_names = []
    with open('pv_names.csv', newline='') as inputfile:
        for row in csv.reader(inputfile):
            opfobj.pv_names.append(row[0])

    # print(opfobj.pv_names)
    # opfobj.pv_names = np.genfromtxt('pv_names.csv', dtype=None)
    opfobj.pv_inc_matrix = opfobj.pv_inc_matrix.todense()
    ...
    # opfobj.pv_p_init = opfobj.pv_P
    # opfobj.pv_q_init = opfobj.pv_Q
...
def get_sens_matrices(opfobj):
    ...
    """
    # Voltage sensitivity
    # t1 = linalg.inv(opfobj.Y_LL)
    # t2 = np.linalg.inv(np.diag(np.conj(opfobj.vL)))
    # t3 = t1.multiply(csc_matrix(t2))
    # t3 = t1.dot(t2)
    # save_npz('M_vL_base.npz', t3)
    M_vL_pY = load_npz('M_vL_base.npz')
    M_vL_sY = hstack((M_vL_pY, -1j * M_vL_pY))
    # M_vL_sY = np.hstack((M_vL_pY.todense(), -1j * M_vL_pY.todense()))
    ...
    # tt = np.diag(np.abs(opfobj.vL))
    # t4 = np.linalg.pinv(tt)
    # del tt
    # gc.collect()
    # t4 = np.diag(1. / np.abs(opfobj.vL))
    # tt = np.diag(np.conj(opfobj.vL))
    # t5 = np.real(M_vL_pY.multiply(csc_matrix(tt)))
    # K_vMag_pY = t5.multiply(csc_matrix(t4))
    # t5 = np.real(-1j * M_vL_pY.multiply(csc_matrix(tt)))
    # K_vMag_qY = t5.multiply(csc_matrix(t4))
    # save_npz('K_vMag_pY.npz', K_vMag_pY)
    # save_npz('K_vMag_qY.npz', K_vMag_qY)
    # del t4, tt, t5
    # gc.collect()
    K_vMag_pY = load_npz('K_vMag_pY.npz')
    K_vMag_qY = load_npz('K_vMag_qY.npz')
    K_vMag_sY = hstack((K_vMag_pY, K_vMag_qY))
    ...
    # del m_pl_vL_summ, m_pl_vL_yll, m_pl_yll_vl, m_pl_yl0_v0, m_pl_y0l_v0
    # gc.collect()
    # m_pl_sY = vstack((np.real(M_vL_sY), np.imag(M_vL_sY))) @ m_pl_vL[0]
    ...
    # del m_ql_vL_summ, m_ql_vL_yll, m_ql_yll_vl, m_ql_yl0_v0, m_ql_y0l_v0
    # gc.collect()
    # # second rows are the imaginary parts of the loss functions, which can be proven to be zero!
    # # change in reactive power loss to change in injection
    # m_ql_sY = vstack((np.real(M_vL_sY), np.imag(M_vL_sY))) @ m_ql_vL[0]
    # # m_ql_sY = m_ql_vL.dot(np.vstack((np.real(M_vL_sY), np.imag(M_vL_sY))))[0]
    m_pl_sY = np.load('m_pl_sY.npy')
    m_ql_sY= np.load('m_ql_sY.npy')
    # change of line flow "from nodes" to change in voltage
    # M_if_vL = opfobj.Yf_L.dot(M_vL_sY)
    # M_if_vL = opfobj.Yf_L @ M_vL_sY
    # opfobj.i_f_abs[np.where(opfobj.i_f_abs == 0.0)[0]] = 1e-3
    # try:
    #     t1 =  np.diag(1. / np.conj(opfobj.i_f_abs))
    #     t2 = np.diag(np.conj(opfobj.i_f)) @ (M_if_vL)
    #     t3 =  np.real(t2)
    #     from scipy import linalg_scipy
    #     M_if_sY = linalg_scipy.blas.dgemm(alpha=1.0, a=t1, b=t3)
    #     # M_if_sY = t1 @ t3
    #     # M_if_sY = t1.multiply(t3)
    ...
    M_if_sY = np.load('M_if_sY.npy')
    logger.debug(f'linear voltage coefficients obtained')
    ...
    logger.debug(f'linear current line flow coefficients obtained')

    opfobj.M_if_pY = M_if_sY[:, 0:len(opfobj.vL)]
    opfobj.M_if_qY = M_if_sY[:, len(opfobj.vL):]

    logger.debug(f'linear active power loss coefficients obtained')

    opfobj.m_pl_pY = m_pl_sY[0:len(opfobj.vL)]
    opfobj.m_pl_qY = m_pl_sY[len(opfobj.vL):]

    logger.debug(f'linear reactive power loss coefficients obtained')

    opfobj.m_ql_pY = m_ql_sY[0:len(opfobj.vL)]
    opfobj.m_ql_qY = m_ql_sY[len(opfobj.vL):]
    logger.debug(f'moving out of the sensitivity model function')
...
#         for k in range(0, len(mult_range)):
#
#             volt_mag_lin_range[:, k] = opfobj.vL_abs_dss + tau * (
#                 np.diag(opfobj.vL_abs_dss).dot(opfobj.reg_inc_matrix)).dot(
...
#
#     cost_a = np.array(u_init) / 100.0
#     cost_b = np.array(u_init) / 1000.0
#     cost_p_gen_a = np.max(cost_a) * 100.0  # promote local production/loadshed
#     cost_p_gen_b = np.mean(cost_b) * 0.1
#
#     # cost_bank_a = np.array([200, 200, 200, 100])/100
#     if len(opfobj.cap_bank_init) > 0:
...
#     if len(opfobj.reg_taps_init) > 0.0:
...
#
#     def cap_max_def_null(m, n):
#         return m.cap_active_number[n] == 0.0
#
...
#
#     def xmer_max_def_null(m, n):
#         return m.xmer_tap_number[n] == 0.0
#
...
#         if len(opfobj.cap_load_inc_matrix) > 0:
...
#             # try this
#             if len(opfobj.tap_info.names) > 0:
#                 xmer_voltage_expr = tau * (np.diag(opfobj.vL_abs_dss).dot(opfobj.reg_inc_matrix)).dot(
#                     opfobj.reg_taps_init - m.xmer_tap_number)
#             else:
#                 xmer_voltage_expr = opfobj.K_vMag_pY.dot(opfobj.flex_load_inc_matrix).dot(m.p_flex_load_var) * 0.0
#
...
#             p_pv_flow_expr = p_pv_flow_matrix.dot(m.p_pv)
#             q_pv_flow_expr = q_pv_flow_matrix.dot(m.q_pv)
#             return m.line_flow_approx[l] == opfobj.i_f_abs[l] + flex_flow_expr[l] \
#                    + p_pv_flow_expr[l] + q_pv_flow_expr[l]
#
#     def obj_rule(m):
...
#
#         if len(opfobj.cap_load_inc_matrix) > 0:
#             cap_obj_term = 0.5 * sum((m.cap_active_number[j] - opfobj.cap_bank_init[j]) * cost_bank_b[j] * (
#                     m.cap_active_number[j] - opfobj.cap_bank_init[j]) for j in cap_node_idx)
#         else:
#             cap_obj_term = 0.0
#
#         if len(opfobj.tap_info.names) > 0:
#             xmr_obj_term = 0.5 * sum((m.xmer_tap_number[j] - opfobj.reg_taps_init[j]) * cost_reg_b[j] * (
...
#     logger.debug(f"no_cap_nodes {no_cap_nodes}")
#     no_xmers_reg = len(opfobj.tap_info.names)
#     logger.debug(f"no_xmers_reg {no_xmers_reg}")
#
#     no_pv_nodes = len(opfobj.pv_p_init)
#
...
#     # logger.debug(f"pv s")
#     # logger.debug(f"{list(opfobj.pv_s_init)}")
#     # logger.debug(f"pv pmax")
#     # logger.debug(f"{list(opfobj.pv_p_init)}")
#     # logger.debug(f"pv qinit")
#     # logger.debug(f"{list(opfobj.pv_q_init)}")
#     # TODO: fix pv_s and pv_p_max mismatch
#     #    q_2 = opfobj.pv_s_init ** 2 - pv_p_max ** 2
#     #     q_2 = opfobj.pv_p_init*0.1
#     #     q_max = np.sqrt(q_2.astype(float))
#     #     pv_q_max = q_max*0.25
#     #     pv_q_min = -q_max*0.25
#     pv_q_max = opfobj.pv_q_init * 0.25
#     pv_q_min = -opfobj.pv_q_init * 0.25
...
#     else:
#         cap_node_idx = range(0, 1)
#         model.cap_active_number = pyo.Var(cap_node_idx)
#         model.cons_cap_eq = pyo.Constraint(cap_node_idx, rule=cap_max_def_null)
#     if no_xmers_reg > 0:
...
#     else:
#         num_xmers_idx = range(0, 1)
#         # xmer variables and constraints
#         model.xmer_tap_number = pyo.Var(num_xmers_idx)  # , domain=pyo.Integers)
#         model.cons_tap_eq = pyo.Constraint(num_xmers_idx, rule=xmer_max_def_null)
#         # model.cons_min_xmer = pyo.Constraint(num_xmers_idx, rule=xmer_min_def)
...
#         else:
#             opfobj.cap_value_opti_abs = np.array([])
...
#         opfobj.active_power_generation_import = np.genfromtxt('base_vals/active_power_generation_import.csv',
#                                                               delimiter=',')
#         opfobj.reactive_power_generation_import = np.genfromtxt('base_vals/reactive_power_generation_import.csv',
#                                                                 delimiter=',')
...
def solve_distributed_optimization(opfobj):
    alpha_p = 1e-5
    alpha_q = 1e-5
    k = 0
    count = 20
    ...
    alpha_mu_v_max = 1e-5
    alpha_mu_v_min = 1e-5
    alpha_mu_i_ij_max = 1e-5
    alpha_mu_i_ij_min = 1e-5
    alpha_mu_p_flex_max = 1e-5
    alpha_mu_p_flex_min = 1e-5
    alpha_mu_p_pv_max = 1e-5
    alpha_mu_p_pv_min = 1e-5
    alpha_mu_q_pv_max = 1e-5
    alpha_mu_q_pv_min = 1e-5
    alpha_mu_cap_max = 1e-5
    alpha_mu_cap_min = 1e-5
    alpha_mu_xmer_max = 1e-5
    alpha_mu_xmer_min = 1e-5
    # some initializing stuff
    # some initializing stuff
    p_flex_load_desired = opfobj.p_flex_load_init / 2
    reg_taps_desired = opfobj.reg_taps_init / 2
    cap_bank_desired = opfobj.cap_bank_init
    pv_p_desired = opfobj.pv_p_init
    pv_q_desired = opfobj.pv_q_init
    line_ratings = opfobj.i_f_abs * 3
    ...
    no_flex_nodes = len(opfobj.flex_info.names)
    logger.debug(f"no_flex_nodes {no_flex_nodes}")
    no_cap_nodes = len(opfobj.cap_info.names)
    logger.debug(f"no_cap_nodes {no_cap_nodes}")
    no_xmers_reg = len(opfobj.tap_info.names)
    logger.debug(f"no_xmers_reg {no_xmers_reg}")
    no_pv_nodes = len(opfobj.pv_p_init.T)
    logger.debug(f"no_pv_nodes {no_pv_nodes}")
    ...
    # cost_bank_a = np.array([200, 200, 200, 100])/100
    if len(opfobj.cap_bank_init) > 0:
        cost_bank_b = 1e-3+opfobj.cap_bank_init / 1000
    else:
        cost_bank_b = 0.0
    cost_q_gen_a = np.mean(cost_bank_b) * 100  # promote local production/capbank activation
    cost_q_gen_b = np.mean(cost_bank_b) * 0.1
    ...
    pv_p_max = np.squeeze(np.asarray(opfobj.pv_p_init)) * 0.25  # will only allow to curtail by quarter of the pv generation
    pv_p_min = pv_p_max * 0.0
    logger.debug(f"pv s")
    logger.debug(f"pv pmax")
    logger.debug(f"{list(opfobj.pv_p_init)}")
    logger.debug(f"pv qinit")
    ...
    #     pv_q_max = q_max*0.25
    #     pv_q_min = -q_max*0.25
    pv_q_max = np.squeeze(np.asarray(opfobj.pv_p_init)) * 0.25
    pv_q_min = -pv_q_max * 0.25
    ...
    voltage_max_lims = opfobj.vL_abs_dss * 1.1
    tau = 0.00625
    p_flex_load_dist = np.zeros(shape=(no_flex_nodes, count + 1))
    p_flex_load_dist[:, 0] = p_flex_load_desired  # initialized to the desired/rated state
    # cap set points
    cap_active_dist = np.zeros(shape=(no_cap_nodes * 3, count + 1))
    cap_active_dist[:, 0] = cap_bank_desired  # initialized to desired/rated state
    # xmer set points
    xmer_tap_dist = np.zeros(shape=(no_xmers_reg, count + 1))
    xmer_tap_dist[:, 0] = 0  # initialized to the desired/rated state
    # pv active power setpoints
    p_pv_dist = np.zeros(shape=(no_pv_nodes, count + 1))
    p_pv_dist[:, 0] = pv_p_desired  # initialized to the desired/rated state
    # pv reactive power setpoint
    q_pv_dist = np.zeros(shape=(no_pv_nodes, count + 1))
    q_pv_dist[:, 0] = pv_q_desired  # initialized to the desired/rated state
    # Dual variables
    # power balance dual
    lambda_p = np.zeros(shape=(1, count + 1))
    lambda_q = np.zeros(shape=(1, count + 1))
    p_net_import_dist = np.zeros(shape=(1, count + 1))
    p_net_import_dist[:, 0] = sum(opfobj.pL) + opfobj.p_loss_dss - sum(pv_p_desired.T)  # initialized to the rated state
    q_net_import_dist = np.zeros(shape=(1, count + 1))
    p_net_import_dist[:, 0] = sum(opfobj.qL) + opfobj.q_loss_dss - sum(
        pv_q_desired.T)  # initialized to the rated state
    # Bus voltage magnitude dual
    mu_v_max = np.zeros(shape=(len(node_idx), count + 1))
    mu_v_min = np.zeros(shape=(len(node_idx), count + 1))

    # Branch Flows duals
    mu_i_ij_max = np.zeros(shape=(len(line_idx), count + 1))
    mu_i_ij_min = np.zeros(shape=(len(line_idx), count + 1))

    # # Generation duals
    # mu_p_flex_max = np.zeros(shape=(no_flex_nodes, count + 1))
    # mu_p_flex_min = np.zeros(shape=(no_flex_nodes, count + 1))
    #
    # # cap limit duals
    # mu_cap_max = np.zeros(shape=(no_cap_nodes, count + 1))
    # mu_cap_min = np.zeros(shape=(no_cap_nodes, count + 1))

    # xmer limit duals
    mu_xmer_max = np.zeros(shape=(no_xmers_reg, count + 1))
    mu_xmer_min = np.zeros(shape=(no_xmers_reg, count + 1))

    # # PV Active power duals
    # mu_p_pv_max = np.zeros(shape=(no_pv_nodes, count + 1))
    # mu_p_pv_min = np.zeros(shape=(no_pv_nodes, count + 1))
    #
    # # PV Reactove power duals
    # mu_q_pv_max = np.zeros(shape=(no_pv_nodes, count + 1))
    # mu_q_pv_min = np.zeros(shape=(no_pv_nodes, count + 1))

    residual_v_max_viol_idx = np.empty(shape=(count + 1), dtype='bool')
    residual_v_min_viol_idx = np.empty(shape=(count + 1), dtype='bool')
    residual_ij_max_viol_idx = np.empty(shape=(count + 1), dtype='bool')
    residual_ij_min_viol_idx = np.empty(shape=(count + 1), dtype='bool')
    # residual_cap_max_viol_idx = np.empty(shape=(count + 1), dtype='bool')
    # residual_cap_min_viol_idx = np.empty(shape=(count + 1), dtype='bool')
    # residual_xmer_max_viol_idx = np.empty(shape=(count + 1), dtype='bool')
    # residual_xmer_min_viol_idx = np.empty(shape=(count + 1), dtype='bool')
    residual_bal_P = np.zeros(shape=(count + 1))
    residual_bal_Q = np.zeros(shape=(count + 1))
    solution_val = np.zeros(shape=(count + 1))

    opfobj.M_if_pY = opfobj.M_if_pY.transpose()
    start_distributed = timeit.default_timer()
    opfobj.K_vMag_pY = opfobj.K_vMag_pY.todense()
    # pv_p_min = np.array(pv_p_max[0])[0]
    # pv_p_min = np.array(pv_p_min[0])[0]
    # pv_q_max = np.array(pv_q_max[0])[0]
    # pv_q_min = np.array(pv_p_min[0])[0]
    while (k < count) and (term == 0):
        # expensive sensitivities used below
        dv_du = opfobj.K_vMag_pY.dot(mu_v_max[:, k] - mu_v_min[:, k])
        """
        ...
        dL_dLambda_P = \
            ...
            + opfobj.m_pl_pY.dot(opfobj.pv_inc_matrix.dot(p_pv_dist[:, k]).T) \
            + opfobj.m_pl_qY.dot(opfobj.pv_inc_matrix.dot(q_pv_dist[:, k]).T)\
            + opfobj.p_loss_dss
        ...
        dL_dLambda_Q = -q_net_import_dist[:, k] \
                       ...
                       + opfobj.m_ql_pY.dot(opfobj.pv_inc_matrix.dot(p_pv_dist[:, k]).T) \
                       + opfobj.m_ql_qY.dot(opfobj.pv_inc_matrix.dot(q_pv_dist[:, k]).T) \
                       + opfobj.q_loss_dss

        #TODO: Add PV Impacts
        dL_dmu_ij_max = max_line_ratings - (opfobj.i_f_abs
                                    + opfobj.M_if_pY.transpose().dot(
            opfobj.flex_load_inc_matrix.dot(p_flex_load_dist[:, k]))
                                    + opfobj.M_if_qY.dot(
            opfobj.cap_load_inc_matrix.dot(cap_active_dist[:, k])))

        dL_dmu_ij_min = (opfobj.i_f_abs + opfobj.M_if_qY.dot(opfobj.cap_load_inc_matrix.dot(cap_active_dist[:, k]))
                         + opfobj.M_if_pY.transpose().dot(
        ...
        - opfobj.flex_load_inc_matrix.transpose().dot(dv_du.T).T
                      # - opfobj.flex_load_inc_matrix.transpose().dot(opfobj.K_vMag_pY.todense()).dot(mu_v_max[:, k] - mu_v_min[:, k])
        # + (mu_p_flex_max[:, k] - mu_p_flex_min[:, k])

        p_flex_load_dist_temp = p_flex_load_desired + np.diag(np.array(1 / (cost_b))).dot(temp_p_flex.transpose()).transpose()

        # capacitor banks
        temp_q_cap = -lambda_q[:, k] * np.ones(shape=(len(cap_node_idx)*3)) + \
             + lambda_p[:, k] * opfobj.m_pl_qY.dot(opfobj.cap_load_inc_matrix) \
             + lambda_q[:, k] * opfobj.m_ql_qY.dot(opfobj.cap_load_inc_matrix) \
                                  - opfobj.cap_load_inc_matrix.transpose().dot(dv_du.T).T \
             - opfobj.cap_load_inc_matrix.transpose().dot(opfobj.M_if_qY.transpose()).dot(
            mu_i_ij_max[:, k] - mu_i_ij_min[:, k])

        #  - opfobj.cap_load_inc_matrix.transpose().dot(opfobj.K_vMag_qY.todense()).dot(
        # mu_v_max[:, k] - mu_v_min[:, k]) \
        # + (mu_cap_max[:, k] - mu_cap_min[:, k])

        cap_active_dist_temp = cap_bank_desired + np.diag(np.array(1 / (cost_bank_b))).dot(temp_q_cap.transpose()).transpose()
        ...
        temp_p_pv = -np.array(cost_pv_p_a) + lambda_p[:, k] * opfobj.m_pl_pY.dot(
            opfobj.pv_inc_matrix) \
                    - lambda_p[:, k] * np.ones(shape=(no_pv_nodes)) + lambda_q[:, k] * opfobj.m_ql_pY.dot(
            opfobj.pv_inc_matrix) \
                    - opfobj.pv_inc_matrix.transpose().dot(opfobj.M_if_pY).dot(
            mu_i_ij_max[:, k] - mu_i_ij_min[:, k]) \
                    - opfobj.pv_inc_matrix.transpose().dot(dv_du.T).T
        # - opfobj.pv_inc_matrix.transpose().dot(opfobj.K_vMag_pY.todense()).dot(mu_v_max[:, k] - mu_v_min[:, k])

        p_pv_dist_temp = pv_p_desired + np.diag(np.array(1 / (cost_pv_p_b))).dot(temp_p_pv)

        # flex_load reactive power updates
        temp_q_pv = -lambda_q[:, k] * np.ones(shape=(no_pv_nodes)) + \
                    + lambda_p[:, k] * opfobj.m_pl_qY.dot(opfobj.pv_inc_matrix) \
                    + lambda_q[:, k] * opfobj.m_ql_qY.dot(opfobj.pv_inc_matrix) \
                    - opfobj.pv_inc_matrix.transpose().dot(dv_du.T).T \
                    - opfobj.pv_inc_matrix.transpose().dot(opfobj.M_if_qY.transpose()).dot(
            mu_i_ij_max[:, k] - mu_i_ij_min[:, k])

        q_pv_dist_temp = pv_q_desired + np.diag(np.array(1 / (cost_pv_q_b))).dot(temp_q_pv)

        # generation imports
        p_net_import_dist[:, k + 1] = (-lambda_p[:, k] - cost_p_gen_a) / 100
        q_net_import_dist[:, k + 1] = (-lambda_q[:, k] - cost_q_gen_a) / 10

        p_flex_load_dist_temp = np.array(p_flex_load_dist_temp[0])[0]
        q_pv_dist_temp = np.array(q_pv_dist_temp[0])[0]
        p_pv_dist_temp = np.array(p_pv_dist_temp[0])[0]
        cap_active_dist_temp =  np.array(cap_active_dist_temp[0])[0]
        ...
        max_viol_mask = [p_flex_load_dist_temp[i] > p_flex_load_var_limit_max[i] for i in
                         range(0, len(p_flex_load_var_limit_max))]
        min_viol_mask = [p_flex_load_dist_temp[i] < p_flex_load_var_limit_min[i] for i in
                         range(0, len(p_flex_load_var_limit_min))]
        ...
        """
            ...
            def power_balance_def_P(m):
                ...
                return m.p_net_import \
                       ...
                       + opfobj.p_loss_dss
        """
        residual_bal_P[k + 1] = dL_dLambda_P
        ...
        residual_bal_Q[k + 1] = dL_dLambda_Q
        ...
        residual_v_max_viol_idx[k] =any(np.squeeze(np.asarray(residual_v_max)) <= 0)
        residual_v_min_viol_idx[k] = any(np.squeeze(np.asarray(residual_v_min)) >= 0)
        residual_ij_max_viol_idx[k] = any(np.squeeze(np.asarray(residual_i_ij_max)) <= 0)
        residual_ij_min_viol_idx[k] = any(np.squeeze(np.asarray(residual_i_ij_min)) >= 0)

        obj_term1 = (p_flex_load_dist[:, k] - opfobj.p_flex_load_init / 2).dot(cost_a) \
                     + 0.5 * (p_flex_load_dist[:, k] - opfobj.p_flex_load_init / 2).dot(np.diag(cost_b)).dot(
            p_flex_load_dist[:, k] - opfobj.p_flex_load_init / 2)

        obj_term2 = cost_p_gen_a * p_net_import_dist[:, k] \
                                           + 0.5 * p_net_import_dist[:, k]*cost_p_gen_b *  p_net_import_dist[:, k] \
                                           + cost_q_gen_a * q_net_import_dist[:, k] \
                                           + 0.5 * q_net_import_dist[:, k]*cost_q_gen_b *  q_net_import_dist[:, k]
        obj_term3 = (p_pv_dist[:, k] - np.squeeze(np.asarray(opfobj.pv_p_init))).dot(
            np.squeeze(np.asarray(cost_pv_p_a.T))) \
                    + 0.5 * (p_pv_dist[:, k] - np.squeeze(np.asarray(opfobj.pv_p_init))).dot(
            np.diag(np.squeeze(np.asarray(cost_pv_p_b)))).T.dot(
            p_pv_dist[:, k] - np.squeeze(np.asarray(opfobj.pv_p_init))) \
                    + 0.5 * (q_pv_dist[:, k] - np.squeeze(np.asarray(opfobj.pv_q_init))).dot(
            np.diag(np.squeeze(np.asarray(cost_pv_q_b)))).T.dot(
            q_pv_dist[:, k] - np.squeeze(np.asarray(opfobj.pv_q_init)))

        solution_val[k] = obj_term1 + obj_term2 + obj_term3
    ...
    iteration_times = k
    min_solution_step = np.argmin(solution_val[0:k-1])
    logger.debug(f'optimization algorithm finished ---')
    logger.debug(f'starting obj_solution_value {solution_val[k-1]} --- max obj value seen {min(solution_val[0:k-1])} min obj value seen {max(solution_val[0:k-1])}---')
    logger.debug(f'minimum solution value {solution_val[min_solution_step]} found at step {min_solution_step} ---')
    logger.debug(f'minimum solution value {solution_val[min_solution_step]} found at step {min_solution_step} ---')
    logger.debug(f"solution value series {list(solution_val)}")
    logger.info(f'optimization algorithm finished ---')
    logger.info(f'starting obj_solution_value {solution_val[k]} --- max obj value seen {min(solution_val[0:k-1])} min obj value seen {max(solution_val[0:k-1])}---')


    opfobj.p_flex_load_var_k = p_flex_load_dist[:, min_solution_step]
    opfobj.cap_value_k = cap_active_dist[:, min_solution_step]
    opfobj.p_pv_k = p_pv_dist[:, min_solution_step]
    opfobj.q_pv_k = q_pv_dist[:, k-1]

    opfobj.p_flex_load_var_opti = 1e3*opfobj.p_flex_load_init - opfobj.p_flex_load_var_k
    opfobj.cap_value_opti = 1e3*opfobj.cap_bank_init - opfobj.cap_value_k
    opfobj.p_pv_opti = 1e3*opfobj.pv_p_init - opfobj.p_pv_k
    opfobj.q_pv_opti = 1e3*opfobj.pv_q_init - opfobj.q_pv_k