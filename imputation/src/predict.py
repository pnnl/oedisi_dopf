...
from train import PowerSystemInjectionPredictor as InjPred
...
def detailed_comparison_stats(array1, array2, array_names=['Array 1', 'Array 2']):
    ...
    for phase in range(3):
        ...
        print(
            f"{array_names[0]:15}",
            f"Mean: {np.mean(arr1[:, phase]):8.4f}",
            f"Std: {np.std(arr1[:, phase]):8.4f}"
        )
        print(
            f"{array_names[1]:15}",
            f"Mean: {np.mean(arr2[:, phase]):8.4f}",
            f"Std: {np.std(arr2[:, phase]):8.4f}"
        )
        ...
        print(
            f"{'Difference':15}",
            f"Mean: {np.mean(diff):8.4f}",
            f"Std: {np.std(diff):8.4f}"
        )
        print(
            f"{'Abs Difference':15}",
            f"Mean: {np.mean(abs_diff):8.4f}",
            f"Max: {np.max(abs_diff):8.4f}"
        )
        ...
        print(
            f"{'% Difference':15}",
            f"Mean: {np.mean(percent_diff):8.2f}%",
            f"Max: {np.max(percent_diff):8.2f}%"
        )
...
if __name__ == "__main__":
    ...
    input, model, output = init_paths(args)
    ...
    agentFilePath = f"{model}/power_system_injection_predictor.h5"

    predictor = InjPred(n_buses, n_phases, model)
