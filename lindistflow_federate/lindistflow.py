...
def voltage_cons_sec(A, b, p, frm, to, counteq, p_pri, q_pri, p_sec, q_sec, nbus_ABC, nbus_s1s2, nbranch_ABC, nbranch_s1s2):
    A[counteq, frm] = 1
    A[counteq, to] = -1
...
def optimal_power_flow(branch_info: dict, bus_info: dict, slack_bus: str,  relaxed: bool):
    ...
    for k, val_br in branches.items():
        ...
        if "REG" != val_br.tag:
            ...
            A_eq, b_eq = voltage_cons_pri(
                A_eq, b_eq, idx, val_br.fr_idx, val_br.to_idx,
                counteq, paa, qaa, pab, qab, pac, qac, nbus_ABC, nbus_s1s2, nbranch_ABC, baseZ)
            counteq += 1
            ...
            A_eq, b_eq = voltage_cons_pri(
                A_eq, b_eq, idx, nbus_ABC + val_br.fr_idx, nbus_ABC + val_br.to_idx,
                counteq, pba, qba, pbb, qbb, pbc, qbc, nbus_ABC, nbus_s1s2, nbranch_ABC, baseZ)
            counteq += 1
    ...
    for k, val_br in branches.items():
        ...
        if val_br.tag == 'SPLIT_PHASE':
            ...
            A_eq, b_eq = voltage_cons_sec(
                A_eq, b_eq, idx - nbranch_ABC, from_bus, to_bus, counteq,
                p_pri, q_pri, p_sec, q_sec, nbus_ABC, nbus_s1s2, nbranch_ABC, baseZ)
            counteq += 1
            ...
            # Monish
            A_eq, b_eq = voltage_cons_sec(
                A_eq, b_eq, idx - nbranch_ABC, from_bus, to_bus, counteq, p_s1,
                q_s1, p_s2, q_s2, nbus_ABC, nbus_s1s2, nbranch_ABC, baseZ)
            counteq += 1
        idx += 1
    ...
    for k in range(n_flow_ABC, n_flow_ABC + nbranch_ABC):
        if f"{from_bus[i]}.1" not in line_flow:
            line_flow[f"{from_bus[i]}.1"] = [0.0, 0.0]
        if f"{from_bus[i]}.2" not in line_flow:
            line_flow[f"{from_bus[i]}.2"] = [0.0, 0.0]
        if f"{from_bus[i]}.3" not in line_flow:
            line_flow[f"{from_bus[i]}.3"] = [0.0, 0.0]
        line_flow[f"{from_bus[i]}.1"] = np.add(line_flow[f"{from_bus[i]}.1"], [x.value[k] * mul * 1000,
                                                                               x.value[k + nbranch_ABC * 3] * mul * 1000]).tolist()
        line_flow[f"{from_bus[i]}.2"] = np.add(line_flow[f"{from_bus[i]}.2"], [x.value[k + nbranch_ABC] *
                                                                               mul * 1000, x.value[k + nbranch_ABC * 4] * mul * 1000]).tolist()
        line_flow[f"{from_bus[i]}.3"] = np.add(line_flow[f"{from_bus[i]}.3"], [x.value[k + nbranch_ABC * 2] * mul * 1000,
                                                                               x.value[k + nbranch_ABC * 5] * mul * 1000]).tolist()
        i += 1
    ...
    for key, val_bus in buses.items():
        bus_voltage[f"{key}.1"] = math.sqrt(abs(x.value[val_bus.idx]*1000))
        bus_voltage[f"{key}.2"] = math.sqrt(
            abs(x.value[nbus_ABC + val_bus.idx]*1000))
        bus_voltage[f"{key}.3"] = math.sqrt(
            abs(x.value[nbus_ABC * 2 + val_bus.idx]*1000))
        i += 1
