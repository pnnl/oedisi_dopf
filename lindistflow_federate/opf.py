...
5. Q control added
6. OPF runs on pu input values

"""


# Optimization to regulate the voltage
def _solve_lindist_OPF(
    branch_sw_data_case,
    bus_info,
    kw_converter,
    source_bus,
    ...
    P_control=True,
    Q_control=False,
    print_LineFlows_Voltage=False,
    print_result=True,
    primary_kv_level=0.01,
):
    ...
    def update_ratios(bus: dict, branch: dict) -> dict:
        for node in branch.values():
            if "XFMR" == node["type"]:
                ...
                bus[src]["tap_ratio"] = 0
                bus[dst]["tap_ratio"] = 1 / r

        return bus

    bus_info = update_ratios(bus_info, branch_sw_data_case)
    ...
    for k, val_br in branch_sw_data_case.items():
        ...
        if val_br["type"] not in secondary_model:
            if val_br["type"] != "XFMR":
                ...
                counteq += 1
            elif val_br["type"] == "XFMR":
                tap_ratio = bus_info[val_br["to_bus"]]["tap_ratio"]
                # Phase A
                A_eq[counteq, val_br["from"]] = tap_ratio**2
                A_eq[counteq, val_br["to"]] = -1
                b_eq[counteq] = 0.0
                counteq += 1

                # Phase B
                A_eq[counteq, nbus_ABC + val_br["from"]] = tap_ratio**2
                A_eq[counteq, nbus_ABC + val_br["to"]] = -1
                b_eq[counteq] = 0.0
                counteq += 1

                # Phase C
                A_eq[counteq, nbus_ABC * 2 + val_br["from"]] = tap_ratio**2
                A_eq[counteq, nbus_ABC * 2 + val_br["to"]] = -1
                b_eq[counteq] = 0.0
                counteq += 1
    ...
    if P_control == True and Q_control == False:
        ...
        for keyb, val_bus in bus_info.items():
            if keyb != source_bus:
                ...
                #     counteq += 1
                #     # Reactive power
                #     A_eq[counteq, nbus_ABC * 3 + nbus_s1s2 + nbus_ABC * 6 + nbus_s1s2 + val_bus['idx']] = 1
                #     A_eq[counteq, n_Qdg + nbus_ABC * 2 + val_bus['idx']] = -1
                #     b_eq[counteq] = val_bus['pq'][1] * baseS
                #     counteq += 1
                #
                #     DG_up_lim[nbus_ABC * 2 + val_bus['idx']] = val_bus['pv'][0] * baseS
            ...
            if keyb != source_bus:
                ...
                if val_bus["kv"] > primary_kv_level:
                    ...
                    DG_up_lim[nbus_ABC * 0 + val_bus["idx"]] = np.sqrt(
                        ((S_capacity * val_bus["pv"][0][0] * baseS) ** 2)
                        - ((val_bus["pv"][0][0] * baseS) ** 2)
                    )
                    DG_up_lim[nbus_ABC * 1 + val_bus["idx"]] = np.sqrt(
                        ((S_capacity * val_bus["pv"][1][0] * baseS) ** 2)
                        - ((val_bus["pv"][1][0] * baseS) ** 2)
                    )
                    DG_up_lim[nbus_ABC * 2 + val_bus["idx"]] = np.sqrt(
                        ((S_capacity * val_bus["pv"][2][0] * baseS) ** 2)
                        - ((val_bus["pv"][2][0] * baseS) ** 2)
                    )

                    # DG_up_lim[nbus_ABC * 0 + val_bus['idx']] = np.sqrt(
                    #     ((S_capacity * val_bus['s_rated'][0] * baseS) ** 2) - ((val_bus['pv'][0][0] * baseS) ** 2))
                    # DG_up_lim[nbus_ABC * 1 + val_bus['idx']] = np.sqrt(
                    #     ((S_capacity * val_bus['s_rated'][1] * baseS) ** 2) - ((val_bus['pv'][1][0] * baseS) ** 2))
                    # DG_up_lim[nbus_ABC * 2 + val_bus['idx']] = np.sqrt(
                    #     ((S_capacity * val_bus['s_rated'][2] * baseS) ** 2) - ((val_bus['pv'][2][0] * baseS) ** 2))
    ...
    for keyb, val_bus in bus_info.items():
        if val_bus["kv"] < primary_kv_level:
            ...
            A_eq[counteq, n_Qdg + nbus_ABC * 2 + val_bus["idx"]] = 1
            b_eq[counteq] = 0.0 * val_bus["s_rated"] * baseS
            counteq += 1
    ...
    if print_LineFlows_Voltage == True:
        print(
            tabulate(
                flow,
                headers=["Line Name", "from", "to", "P_S1S2", "Q_S1S2"],
                tablefmt="psql",
            )
        )
    ...
    for k in range(nbus_ABC):
        volt.append(
            [
                name[k],
                "{:.4f}".format(math.sqrt(abs(x.value[k]))),
    ...
    if print_result:
        ...
        if P_control:
            print(
                ...
                + str(sum(sum(generation_output)) * kw_converter)
                + " kW"
            )
        elif Q_control:
            print(
        ...
        print("Solver Time " + str(prob.solver_stats.solve_time) + " seconds")
        print("Problem solved with " + str(prob.solver_stats.num_iters) + " iterations")
        print("---------------------------%%%%%--------------------------")

    return bus_voltage, line_flow, opf_control_variable
